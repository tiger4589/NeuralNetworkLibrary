...
namespace FeedForwardNeuralNetwork.Core
{
    public class FeedForwardNeuralNet
    {
        ...
        public FeedForwardNeuralNet(int inputLayerNeuronsNumber, int hiddenLayerNeuronsNumber,
        ...
        internal FeedForwardNeuralNet(Layer inputLayer, Layer hiddenLayer, Layer outputLayer, double learningRate)
        {
        ...
        /// <summary>
        /// Trains the network to classify inputs (normal back propagation, no SGD)
        /// </summary>
        /// <param name="listOfInputs">List of Inputs</param>
        /// <param name="listOfOutputs">List of corresponding outputs</param>
        public void Train(List<double[]> listOfInputs, List<double[]> listOfOutputs)
        {
            for (int i = 0; i < listOfInputs.Count; i++)
            {
                Train(listOfInputs[i], listOfOutputs[i]);
            }
        }

        private void Train(double[] input, double[] output)
        {
            Run(input); // we have to run the network through the input so we can compute the error

            int outputNeuronsCount = OutputLayer.Neurons.Length;

            //start going backward to hidden layer
            for (int i = 0; i < outputNeuronsCount; i++)
            {
                double x = OutputLayer.Neurons[i].Value;
                OutputLayer.Neurons[i].Delta = SigmoidDerivative(x) * (output[i] - x);
                OutputLayer.Neurons[i].Bias += LearningRate * OutputLayer.Neurons[i].Delta;

                for (int j = 0; j < HiddenLayer.Neurons.Length; j++)
                {
                    HiddenLayer.Neurons[j].Delta = SigmoidDerivative(HiddenLayer.Neurons[j].Value) *
                                                   OutputLayer.Neurons[i].Weights[j].Value *
                                                   OutputLayer.Neurons[i].Delta;
                    OutputLayer.Neurons[i].Weights[j].Value += LearningRate * HiddenLayer.Neurons[j].Value *
                                                                  OutputLayer.Neurons[i].Delta;
                    HiddenLayer.Neurons[j].Bias += LearningRate * HiddenLayer.Neurons[j].Delta;

                    for (int k = 0; k < HiddenLayer.Neurons[i].Weights.Length; k++)
                    {
                        HiddenLayer.Neurons[j].Weights[k].Value += LearningRate * InputLayer.Neurons[k].Value *
                                                                      HiddenLayer.Neurons[j].Delta;
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Train the neural network using SGD algorithm to classify the given inputs
        /// </summary>
        ...
        public void Train(List<double[]> listOfInputs, List<double[]> listOfOutputs, int numberOfEpochs, int miniBatchSize = 10, double lambda = 5.0,
        ...
        public static FeedForwardNeuralNet Load(string fileName)
        {
            string data;
            ...
            FeedForwardNeuralNet net = convertor.DeserializeData();
            return net;
        }
    }
}